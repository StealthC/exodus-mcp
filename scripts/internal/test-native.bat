...
cl /nologo /EHsc /W4 /std:c++17 /I "%ROOT%native-plugin" "%ROOT%native-plugin\BridgeWire.cpp" "%ROOT%native-plugin\tests\test_bridge_wire.cpp" /Fe:"%ROOT%bin\test-bridge-wire.exe"
if errorlevel 1 goto fail
"%ROOT%bin\test-bridge-wire.exe"
set "TEST_EXIT=%ERRORLEVEL%"
del "%ROOT%bin\test-bridge-wire.exe" >nul 2>&1
exit /b %TEST_EXIT%
